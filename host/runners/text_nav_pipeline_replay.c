...
static int read_replay_config(char *line, int line_size, struct text_pipeline_fixture *fixture) {
...
static int init_pipeline(struct text_pipeline_fixture *fixture) {
...
static int process_motion_event(struct text_pipeline_fixture *fixture, uint32_t timestamp,
                                int32_t x, int32_t y) {
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        if (process_motion_event(&fixture, timestamp, x, y) < 0) {
            return 2;
        }
    }
    return 0;
