...
export function validateFixture(fixture, path = "fixture") {
  ...
  if (!fixture.id || !["composed-scroll", "composed-text", "composed-noise",
    "cursor-pipeline"].includes(fixture.processor?.kind)) {
    throw new Error(`${path}: id and a supported pipeline kind are required`);
  }

  const settings = fixture.processor.settings;
  if (fixture.processor.kind === "composed-scroll") {
...
export function encodeRunnerInput(fixture) {
  const settings = fixture.processor.settings;
  const configuration = fixture.processor.kind === "composed-scroll"
