 ...
 * builds; the upstream quantizer owns rounding and remainders.
 */
...
static int cursor_sink_emit(struct zpt_sink *sink, const struct zpt_signal *signal) {
    ...
    int32_t x = signal->data.delta.x;
    int32_t y = signal->data.delta.y;
    if (x < INT16_MIN || x > INT16_MAX || y < INT16_MIN || y > INT16_MAX) {
        return -ERANGE;
    }

    int ret = input_report(config->output_device, INPUT_EV_REL, INPUT_REL_X, x, false, K_NO_WAIT);
    if (ret < 0) {
        return ret;
    }
    return input_report(config->output_device, INPUT_EV_REL, INPUT_REL_Y, y, true, K_NO_WAIT);
}
